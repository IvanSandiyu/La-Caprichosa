...
def _club_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT DISTINCT p.player_id, p.name, p.image_url
        FROM player_clubs pc
        JOIN players p ON p.player_id = pc.player_id
        WHERE pc.club_id = ? AND {_img('p')}
        """,
        (info["id"],),
    ).fetchall()
...
def _position_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT player_id, name, image_url
        FROM players WHERE position = ? AND {_img()}
        """,
        (info["label"],),
    ).fetchall()
    return [{"id": r[0], "name": r[1], "image_url": r[2]} for r in rows]
...
def _nationality_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT DISTINCT p.player_id, p.name, p.image_url
        FROM player_countries pc
        JOIN players p ON p.player_id = pc.player_id
        WHERE pc.norm = ? AND {_img('p')}
        """,
        (info["label"],),
    ).fetchall()
    return [{"id": r[0], "name": r[1], "image_url": r[2]} for r in rows]
...
def _birth_year_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT player_id, name, image_url
        FROM players
        WHERE substr(dob,1,4) = ? AND {_img()}
        """,
        (str(info["label"]),),
    ).fetchall()
    return [{"id": r[0], "name": r[1], "image_url": r[2]} for r in rows]
...
def _last_name_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT player_id, name, image_url
        FROM players
        WHERE name LIKE '%' || ? || '%' AND {_img()}
        """,
        (info["label"],),
    ).fetchall()
    return [{"id": r[0], "name": r[1], "image_url": r[2]} for r in rows]
...
def _two_clubs_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _club_country_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _born_before_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _youth_club_players(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _club_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT DISTINCT p.player_id, p.name, p.image_url
        FROM players p
...
def _position_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT player_id, name, image_url
        FROM players
        WHERE {_img()} AND position IS NOT NULL AND position != ?
...
def _nationality_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT DISTINCT p.player_id, p.name, p.image_url
        FROM players p
...
def _birth_year_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT player_id, name, image_url
        FROM players
        WHERE {_img()} AND substr(dob,1,4) != ?
        ORDER BY player_id LIMIT ?
        """,
...
def _last_name_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
    rows = conn.execute(
        f"""
        SELECT player_id, name, image_url
        FROM players
        WHERE {_img()} AND name NOT LIKE '%' || ? || '%'
        ORDER BY player_id LIMIT ?
        """,
        (info["label"], limit),
    ).fetchall()
...
def _two_clubs_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _club_country_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _born_before_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _youth_club_impostors(conn: sqlite3.Connection, info: dict, limit: int) -> list[dict]:
...
def _generate(
    ...
    for info in catalog:
        ...
        correct_pool = query_fn(conn, info, BOARD_SIZE)
        ...
        impostor_pool = impostor_fn(conn, info, pool_limit)
